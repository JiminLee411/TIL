...
HTMl에서 사용자로부터 저오를 받기 위해서는`form`태그를 활용한다.
...
<form action="/pong">
    ...
    <input type+"subit" value="전송">
</form>
```

* form 태그는 `action` 속성으로 해당 폼이 전송될 url을 지정해야한다.
* form 태그 내에는 `input` 태그들을 정의하여, 사용자에세 받으 정보를(설문지를) 만들어 놓는다.
...
2. 정보 받아서 활용하기
   ...
   ```python
   # app.py
   from flask import Flask, render_teplate, request
